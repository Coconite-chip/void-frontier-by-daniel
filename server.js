...
// ── Stars state ──────────────────────────────────────────────────
// Mirrors the client's solarObjects array (same seed, same positions)
// We only track mutable state: destroyed, colonized, owner
const starStates = new Map(); // starName → { destroyed, ownerId, ownerName, colonizedAt }
const STAR_SPAWN_INTERVAL = 30000; // 30s
const STAR_MAX = 20;
const STAR_MIN_DIST = 400; // min distance between stars
const WORLD = 4000;

// Star name pool (same as client)
const STAR_NAMES_POOL = [
  'HELIOS-I','PYROS-VII','IGNIS-PRIME','SOLUM-IX','CINDER-GATE','FLARE-OMEGA','CORONA-III',
  'SOLAR-VIII','PYRE-NEXUS','INFERNO-IV','EMBER-GATE','FLARE-BELT','CORONA-OMEGA','RADIANT-IX',
  'SOLUM-II','IGNIS-VI','BLAZEN-PRIME','SOLIS-MINOR','PYRA-HAVEN','HELIOS-REACH'
];
const STAR_COLORS = ['#ffaa00','#ff6b35','#ffdd00','#ff4400','#ffa500'];

// Seeded RNG (same as client — Mulberry32)
let _seedState = 0x6F726F6E;
function seededRand() {
...
function srnd(a, b) { return a + seededRand() * (b - a); }

// Active stars list (live, can grow/shrink)
const activeStars = []; // { name, x, y, r, color, pulseT }

// Generate initial 7 stars matching the client seed exactly
function initStars() {
  // Reset seed to match client's seedSolarObjects call
  _seedState = 0x6F726F6E;
  // Skip ahead past seedUniverse calls (client calls seedUniverse first)
  // We'll just use fixed positions matching the seeded output
  const starNames = ['HELIOS-I','PYROS-VII','IGNIS-PRIME','SOLUM-IX','CINDER-GATE','FLARE-OMEGA','CORONA-III'];
  for (let i = 0; i < 7; i++) {
    const star = {
      name: starNames[i],
      x: srnd(-WORLD * 0.75, WORLD * 0.75),
      y: srnd(-WORLD * 0.75, WORLD * 0.75),
      r: srnd(35, 65),
      color: STAR_COLORS[Math.floor(srnd(0, 5))],
      pulseT: srnd(0, Math.PI * 2),
    };
    activeStars.push(star);
  }
  console.log('[S] Initialized', activeStars.length, 'stars');
}

function spawnNewStar() {
  const alive = activeStars.filter(s => {
    const st = starStates.get(s.name);
    return !st || !st.destroyed;
  });
  if (alive.length >= STAR_MAX) return;

  // Pick a name not already used
  const usedNames = new Set(activeStars.map(s => s.name));
  const available = STAR_NAMES_POOL.filter(n => !usedNames.has(n));
  ...
  do {
    ...
    y = srnd(-WORLD * 0.75, WORLD * 0.75);
    tries++;
  } while (tries < 30 && alive.some(s => Math.hypot(s.x - x, s.y - y) < STAR_MIN_DIST));
  ...
  activeStars.push(star);
  io.emit('star:spawned', star);
  console.log('[S] New star spawned:', name, 'at', Math.round(x), Math.round(y), '— total alive:', alive.length + 1);
}

initStars();
setInterval(spawnNewStar, STAR_SPAWN_INTERVAL);
...
io.on('connection', (socket) => {
  ...
  socket.on('arena:queue', ({ bet }) => {
    // Auto-register if player not yet registered (reconnect edge case)
    if (!players.has(socket.id)) {
      players.set(socket.id, {
        id: socket.id, name: 'Pilot-' + socket.id.slice(0, 4),
        color: '#00d4ff', x: 0, y: 0, angle: 0, hp: 100, credits: 2000, groupId: null,
      });
    }
    const p = players.get(socket.id);
    ...
    // Don't queue if already in active arena
    for (const [, arena] of arenas) {
      if (arena.active && (arena.p1.id === socket.id || arena.p2.id === socket.id)) {
        socket.emit('error', { msg: 'Ya estás en una arena activa.' });
        return;
      }
    }
  ...
  // ── Disconnect ───────────────────────────────────────────────
  // ── STAR: Destroy ────────────────────────────────────────────
  socket.on('star:destroy', ({ starName }) => {
    const p = players.get(socket.id);
    const existing = starStates.get(starName);
    if (existing && existing.destroyed) return; // already destroyed
    if (existing && existing.ownerId) {
      socket.emit('error', { msg: 'No puedes destruir una estrella colonizada.' });
      return;
    }
    starStates.set(starName, { destroyed: true, ownerId: null, ownerName: null });
    io.emit('star:destroyed', { starName, destroyedBy: p ? p.name : '?' });
    console.log('[S] Star destroyed:', starName, 'by', socket.id);
  });

  // ── STAR: Colonize ────────────────────────────────────────────
  socket.on('star:colonize', ({ starName }) => {
    const p = players.get(socket.id);
    if (!p) return;
    const existing = starStates.get(starName);
    if (existing && existing.destroyed) {
      socket.emit('error', { msg: 'Esta estrella ya fue destruida.' });
      return;
    }
    if (existing && existing.ownerId) {
      socket.emit('error', { msg: 'Esta estrella ya está colonizada por ' + existing.ownerName + '.' });
      return;
    }
    // Check if player already owns a star
    for (const [, st] of starStates) {
      if (st.ownerId === socket.id) {
        socket.emit('error', { msg: 'Ya tienes una estrella colonizada. Solo puedes tener 1.' });
        return;
      }
    }
    starStates.set(starName, { destroyed: false, ownerId: socket.id, ownerName: p.name, colonizedAt: Date.now() });
    io.emit('star:colonized', { starName, ownerId: socket.id, ownerName: p.name });
    console.log('[S] Star colonized:', starName, 'by', p.name);
  });

  // ── STAR: Request full state ──────────────────────────────────
  socket.on('stars:request', () => {
    socket.emit('stars:state', {
      stars: activeStars,
      states: Array.from(starStates.entries()).map(([name, st]) => ({ name, ...st })),
    });
  });
  ...
  function createArena(p1Entry, p2Entry) {
    ...
    if (!p1 || !p2) {
      console.log('[A] createArena failed: missing player', p1Entry.id, p2Entry.id);
      return;
    }
    ...
    console.log('[A] Arena created:', arenaId, p1.name, 'vs', p2.name, '— auto-start in 8s');

    // Auto-start after 8 seconds regardless of ready state
    arena.autoStartTimer = setTimeout(() => {
      startArena(arenaId);
    }, 8000);
  }

  // ── Internal: start arena ────────────────────────────────────
  function startArena(arenaId) {
    const arena = arenas.get(arenaId);
    if (!arena || arena.started) return;
    if (arena.autoStartTimer) { clearTimeout(arena.autoStartTimer); arena.autoStartTimer = null; }
    arena.started = true;
    arena.active = true;
