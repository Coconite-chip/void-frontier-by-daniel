...
function mpConnect() {
  ...
  mp.socket.on('connect', () => {
    ...
    mp.socket.emit('player:register', {
      name: 'Pilot-' + mp.socket.id.slice(0, 4),
      color: player.shipColor || '#00d4ff',
    });
    addLog('🌐 Multijugador conectado. ID: ' + mp.socket.id.slice(0, 6), 'good');
    mpRenderHUD();
  });
  ...
  mp.socket.on('dungeon:entered', ({ dungeonId }) => {
    ...
    mp.dungeonLocks[dungeonId] = { occupied: true, groupLeader: 'tu grupo' };
  });
  ...
  mp.socket.on('dungeon:partner_pos', ({ id, name, color, x, y, dir }) => {
    mp.dungeonPartners[id] = { x, y, dir, name, color };
  });
...
setInterval(() => {
  ...
  const lock = Object.keys(mp.dungeonLocks).find(did => mp.dungeonLocks[did]?.occupied);
  if (!lock) return;
...
window.enterDungeon = function(planet) {
  ...
  if (lock && lock.occupied && lock.groupLeader !== 'tu grupo') {
...
window.exitDungeon = function(victory) {
  if (mp.connected && mp.socket && inDungeon && dungeonPlanet) {
    mp.socket.emit('dungeon:exit', { dungeonId: dungeonPlanet.name });
    mp.dungeonPartners = {};
  }
  ...
  if (tab === 'GRUPO') {
    el.innerHTML = `
      <div style="font-size:9px;color:#4a7a9b;margin-bottom:10px">
        Tu ID: <b style="color:#00ffcc">${mp.myId ? mp.myId.slice(0, 8) : '—'}</b>
        ${mp.connected ? '' : '<span style="color:#ff4444"> (desconectado)</span>'}
      </div>
      ${mp.groupId ? `
...
function mpUpdateArenaLobby() {
  ...
  modal.innerHTML = `
    ...
    <div style="display:flex;gap:8px">
      ...
      <button onclick="mpArenaReady()" style="
        flex:2;background:rgba(255,34,68,0.15);border:1px solid #ff2244;color:#ff2244;
        font-family:Share Tech Mono,monospace;font-size:9px;padding:7px;cursor:pointer;border-radius:3px;letter-spacing:1px
      ">⚔ LISTO PARA COMBATIR</button>
    </div>
  `;
}
...
function mpArenaReady() {
  ...
  addLog('⚔ Listo para combatir! Esperando al rival...', 'warn');
  const btn = document.getElementById('mpArenaLobby')?.querySelector('button:last-child');
  if (btn) { btn.textContent = '⏳ ESPERANDO RIVAL...'; btn.disabled = true; btn.style.opacity = '0.5'; }
}
